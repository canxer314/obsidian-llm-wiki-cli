...
export function createReviewPublisher(options: {
  ...
  return {
    async prepare(checkoutPath: string, branch: string, revision: string): Promise<void> {
      requireFullRevision(revision, "Review publication requires a full expected revision");
      ...
      if (stdout.trim() !== revision) {
        throw new Error("Review checkout did not start at the acquired revision");
      }
    },
    async publish(request: {
      ...
      requireFullRevision(request.expectedRevision, "Review publication requires a full expected revision");
      const { stdout } = await git(["-C", request.checkoutPath, "rev-parse", "HEAD"]);
      const revision = stdout.trim();
      requireFullRevision(revision, "Reviewer did not leave a full local revision");
      ...
      if (remote.length === 0 || /:\/\/[^/]*@/u.test(remote)) {
        throw new Error("Review publication remote is invalid");
      }
      await git([
