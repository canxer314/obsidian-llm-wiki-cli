...
Public failure diagnostics are classifications, not transcripts. Every specific public diagnostic, regardless of the thrown JavaScript value type, is projected by an explicitly trusted runtime producer. The trusted acquisition path authenticates the exact thrown object or function through a private provenance registry and publishes only its registered bounded summary; neither `publicSummary` nor `message`, whether own, inherited, accessor-backed, or introduced through prototype pollution, has authority. The registry exposes no arbitrary-summary registrar. Producer-specific operations derive fixed classifications from runtime-validated inputs for Automation Command validation and outcome failures, Target-operation timeouts, allowlisted Target worker exits, and checkout-command exits. Worker exits and checkout-command failures publish only their coarse classification because their messages embed child-process stderr. Every unregistered thrown value receives a deterministic fallback without property access or coercion, so diagnostic selection cannot expose caller-supplied text or prevent Blocked Automation settlement. Untrusted stderr and any operation-transformed secret stay in local errors and job logs.
