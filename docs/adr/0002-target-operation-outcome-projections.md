...
Public failure diagnostics are classifications, not transcripts. Errors whose messages embed child-process stderr — including Target worker exits caused by checkout-command failures — are projected by an explicit trusted-runtime producer allowlist. The trusted acquisition path authenticates the exact thrown object through a private provenance registry and publishes only its registered coarse summary (still bounded and pattern-redacted as defense in depth); a `publicSummary` property, whether own, inherited, accessor-backed, or introduced through prototype pollution, has no authority. Unknown thrown values are projected without invoking accessors or reflective coercion on objects; malformed objects and Proxies receive a deterministic fallback so diagnostic selection cannot prevent Blocked Automation settlement. Untrusted stderr and any operation-transformed secret stay in local errors and job logs. Free-text pattern redaction is retained only for primitive values and own data-property messages raised entirely by trusted code, because it cannot recognize transformed secrets such as base64-encoded credentials.
