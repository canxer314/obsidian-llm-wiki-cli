...
A new child-process failure surface (worker, checkout command, or similar) must be explicitly added to the trusted producer allowlist before its coarse public summary may reach GitHub publication. A readable property does not establish provenance. Embedding untrusted stderr in a message without an authenticated producer classification is a trust-boundary defect, and no amount of free-text pattern redaction can repair it after the fact.
