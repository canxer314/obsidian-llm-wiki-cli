...
Public failure diagnostics are classifications, not transcripts. Every object-originated public diagnostic is projected by an explicitly trusted runtime producer. The trusted acquisition path authenticates the exact thrown object through a private provenance registry and publishes only its registered bounded summary; neither `publicSummary` nor `message`, whether own, inherited, accessor-backed, or introduced through prototype pollution, has authority. The registry exposes no arbitrary-summary registrar. Producer-specific operations derive fixed classifications from runtime-validated inputs for Automation Command validation and outcome failures, Target-operation timeouts, allowlisted Target worker exits, and checkout-command exits. Worker exits and checkout-command failures publish only their coarse classification because their messages embed child-process stderr. Unknown thrown objects are projected without invoking accessors or reflective coercion; malformed objects and Proxies receive a deterministic fallback so diagnostic selection cannot prevent Blocked Automation settlement. Primitive thrown values remain total, bounded, and pattern-redacted. Untrusted stderr and any operation-transformed secret stay in local errors and job logs.
...
A new child-process failure surface (worker, checkout command, or similar) must add a producer-specific registrar that derives a coarse public classification from runtime-validated inputs. There is no general operation for registering caller-supplied summary text. A readable property does not establish provenance. Embedding untrusted stderr in a message without an authenticated producer classification is a trust-boundary defect, and no amount of free-text pattern redaction can repair it after the fact.
