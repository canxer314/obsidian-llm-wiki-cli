...
The outer Target-operation runner composes this lifecycle only as an owner of bounded process-group execution, whole-job timeouts, and local job-log finalization; operation-specific business behavior stays inside the fixed workers and never moves into the runner. Resolving worker files and fixed operation entries from the trusted automation checkout's `.sandcastle` (see ADR-0001) instead of the operated Target Checkout changes launch authority only: the owner/nested disposition, grace-then-force group termination, timeout execution, and job-log finalization above are unchanged.
