...
A later `run feedback` or Dispatcher dispatch must never adopt a published state: matching current-intent evidence returns typed `feedback-reconciliation` before Agent, checkout, push, or reply. After diagnosis confirms that no matching job is active, follow the normal manual-retry lifecycle first: remove `agent:blocked` and restore `agent:implement`. Only then may the explicit `reconcile feedback` command (including a no-flag invocation) provide the additional authorization to adopt uniquely proven current-intent evidence; it still acquires that visible command through `agent:in-progress`, consumes the trigger, and performs the normal `finally` cleanup. Historical markers for other roots neither satisfy nor poison the selected intent; a strict unique legacy reply remains reconcile-only when the acquired revision is supplied. Reply-only completion additionally requires the supplied reply root to equal the selected intent. An adopted completion reports the independently verified revision and reconciles managed labels; cleanup failures are reflected in the typed result rather than silently ignored.
